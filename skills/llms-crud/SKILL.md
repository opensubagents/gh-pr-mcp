...
Verbs collapse to canonical CRUD: **Create**, **Retrieve** (single), **List** (collection), **Update/Edit**, **Delete**, **Archive**, plus a few resource-specific actions (Add, Send, Stream, Download, Merge, Rebase, Reset).

```
beta/
├── agents/                   create | list | retrieve | archive
│   └── versions/             list
├── sessions/                 create | list | retrieve | delete | archive
│   ├── events                (collection page; user.* events sent here)
│   ├── resources/            add | list | retrieve | delete
│   └── threads/              list | retrieve | archive
│       └── events            (per-thread stream/list)
├── environments/             create | list | retrieve | delete | archive
├── vaults/                   create | list | retrieve | delete | archive
│   └── credentials/          create | list | retrieve | delete | archive
├── files/                    list | retrieve_metadata | download | delete
├── memory_stores/            create | list | retrieve | delete | archive
│   └── memories/             create | delete  (and others — verify on the resource page)
├── skills/                   create | list | retrieve | delete
│   └── versions/             create | list | retrieve | delete
├── user_profiles/            create | retrieve | create_enrollment_url
└── messages/                 (shared with non-beta surface; not exclusive to managed-agents)
```
...
1. **Identify the resource.** agents, sessions, threads (under sessions), events (under sessions or threads), resources (under sessions), environments, vaults, credentials (under vaults), files, memory_stores, memories (under memory_stores), skills, versions (under skills or agents), user_profiles.
2. **Identify the verb.** create, retrieve, list, delete, archive — or a resource-specific action (add, send, stream, download, retrieve_metadata, create_enrollment_url).
3. **Pick the namespace.**
   - "How does X work" / "what is X" → conceptual: `/docs/en/managed-agents/<topic>.md`
   - "How do I call X" / "what's the request shape" → API ref: `/docs/en/api/beta/<resource>[/<sub>]/<verb>.md`
4. **Build the URL** from the pattern. The `.md` suffix is already on every entry in llms.txt; keep it on.
5. **Fetch with `web_fetch`.** Read the canonical first; branch into language variants only on request.
6. **If unsure the URL exists**, fetch llms.txt once and grep — don't guess and fail twice.

## When to escalate to llms.txt directly

Re-fetch `https://platform.claude.com/llms.txt` only when:
- The user names a resource not in this skill's tables (something shipped after v0.1.0).
- The user asks about a namespace outside `/managed-agents/` and `/api/beta/` (see "Out of scope" below).
- A URL constructed from the pattern returned 404 — verify it exists in the index.

Don't fetch llms.txt for routine lookups; the tables here are the index.
